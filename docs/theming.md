...
The user is the last authority. `omarchy theme trust <name>` writes a `.omarchy-trusted` marker into an installed theme, and `omarchy-theme-set` then treats that theme the same as one the user wrote: its Lua and terminal configs stage verbatim. Use it for a theme you have reviewed and vouched for. Trusting a symlinked working copy is a no-op, since a symlink is already the user's own, and a theme that is currently applied needs `omarchy theme set <name>` again for its now-trusted files to reach the running desktop.
